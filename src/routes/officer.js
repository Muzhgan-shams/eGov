...
router.get('/', async (req, res, next) => {
  try {
    const dept = req.user.department_id; // fixed name from passport.deserialize
    const { rows } = await db.query(`
      SELECT r.id, r.status, r.submitted_at,
             s.name AS service, u.name AS citizen_name
      FROM requests r
...
router.get('/requests/:id', async (req, res, next) => {
  try {
    const { rows } = await db.query(`
      SELECT r.*, s.name AS service, s.department_id, u.name AS citizen_name, u.email AS citizen_email
      FROM requests r
      JOIN services s ON s.id = r.service_id
      JOIN users u    ON u.id = r.citizen_id
      WHERE r.id=$1
    ...
    res.render('officer/request', { title:`Request ${item.id}`, item });
  } catch (e) { next(e); }
});

router.post('/requests/:id/decision', async (req, res, next) => {
  try {
    ...
    await db.query(`
      ...
      FROM services s
      WHERE r.id=$2 AND s.id=r.service_id AND s.department_id=$3
