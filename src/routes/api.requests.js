// --- path: src/routes/api.requests.js
...
async function getCitizen(req) {
  ...
  const u = rows[0];
  return (u && u.role === 'CITIZEN') ? u : null;
}

// create request
router.post('/requests', async (req, res) => {
  const me = await getCitizen(req);
...
router.get('/requests', async (req, res) => {
  ...
  const p = [me.id]; let where = 'WHERE r.citizen_id=$1';
...
// upload document (matches schema: filename, mime_type, size_bytes)
router.post('/requests/:id/documents', upload.single('file'), async (req, res) => {
  ...
  await db.query(
    `INSERT INTO documents (request_id, filename, mime_type, size_bytes)
     VALUES ($1,$2,$3,$4)`,
    [req.params.id, req.file.originalname, req.file.mimetype, req.file.size || null]
  );
  res.status(201).json({ ok: true });
});

// simulate payment (matches schema: payments with status PENDING|SUCCESS|FAILED)
router.post('/requests/:id/payments', async (req, res) => {
  const me = await getCitizen(req);
  if (!me) return res.status(401).json({ error:'Unauthenticated' });

  const { rows } = await db.query(`
    SELECT r.id, s.fee_cents
    FROM requests r
    JOIN services s ON s.id = r.service_id
    WHERE r.id=$1 AND r.citizen_id=$2
  `, [req.params.id, me.id]);
